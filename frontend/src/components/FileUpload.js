...
function FileUpload() {
  ...
  return (
    <div>
      ...
      >
        <input {...getInputProps()} />
        <p>드래그 앤 드롭으로 파일을 가져오거나 <strong>여기</strong>를 클릭해서 파일을 선택해주세요</p>
      </div>
      <button
      ...
      {file && (
        <div>
          <h3>업로드 할 파일:</h3>
          <p>{file.name}</p>
          <p><button onClick={handleSubmit}>업로드</button></p>
          <p><button onClick={handleClearFile}>파일 선택 취소</button></p>
        </div>
      )}
    </div>
  );
